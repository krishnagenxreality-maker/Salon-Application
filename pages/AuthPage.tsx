...
const AuthPage: React.FC<AuthPageProps> = ({ 
    ...
    return (
        <div className="w-full min-h-screen relative overflow-x-hidden overflow-y-auto bg-white perspective-[2500px] custom-scrollbar">
            ...
            <div className="relative z-10 w-full min-h-screen flex flex-col md:flex-row items-center justify-center p-4 sm:p-8 md:p-12 lg:p-16">
                
                {/* Branding Block - Responsive Sizing */}
                <div className="w-full md:w-1/2 flex flex-col items-center justify-center text-center py-8 md:py-0">
                    <div className="animate-fade-in flex flex-col items-center">
                        <img 
                            src="/images/logo.png" 
                            alt="Logo" 
                            className="h-8 md:h-10 w-auto mb-6 md:mb-8 brightness-0 invert opacity-80"
                        />
                        <h1 className="text-6xl sm:text-7xl lg:text-9xl font-black text-white tracking-tighter leading-[0.8] drop-shadow-[0_25px_50px_rgba(0,0,0,0.5)] select-none uppercase transition-all">
                            TONI<br/>&GUY
                        </h1>
                        <div className="mt-8 md:mt-10 flex flex-col items-center">
                            <div className="w-12 md:w-16 h-[1px] bg-white/30 mb-4 md:mb-5"></div>
                            <p className="text-white/70 text-[9px] sm:text-xs font-bold tracking-[0.4em] md:tracking-[0.5em] uppercase animate-pulse">
                                Academy Portal
                            </p>
                        </div>
                    </div>
                ...
                <div className="w-full md:w-1/2 flex items-center justify-center mt-4 md:mt-0">
                    ...
                    {/* 3D Flipping Card Container - Responsive Height */}
                    <div className="relative w-full max-w-md h-[580px] sm:h-[620px] preserve-3d transition-transform duration-1000 cubic-bezier(0.4, 0, 0.2, 1)" 
                         style={{ transform: isAdmin ? 'rotateY(180deg)' : 'rotateY(0deg)' }}>
                        
                        {/* FRONT FACE (Candidate) */}
                        <div className="absolute inset-0 backface-hidden bg-white/10 backdrop-blur-2xl p-6 sm:p-10 rounded-[2.5rem] sm:rounded-[3.5rem] shadow-[0_50px_100px_-30px_rgba(0,0,0,0.4)] border border-white/30 flex flex-col">
                            ...
                            <div className="flex-1 flex flex-col mt-6 sm:mt-8">
                                <header className="mb-6 sm:mb-8">
                                    ...
                                    <p className="text-gray-600 text-[10px] sm:text-xs font-bold uppercase">Candidate Portal Access</p>
                                </header>

                                <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-5 flex-grow">
                                    <div className="space-y-1 sm:space-y-1.5">
                                        <label className="text-[9px] sm:text-[10px] font-bold text-black tracking-widest uppercase ml-1">Identifier</label>
                                        <input
                                            type="text"
                                            required
                                            value={userId}
                                            onChange={(e) => setUserId(e.target.value)}
                                            className="block w-full px-5 py-3.5 sm:px-6 sm:py-4 bg-white/70 border border-gray-100 rounded-xl sm:rounded-2xl text-black placeholder-gray-400 focus:outline-none focus:ring-4 focus:ring-black/5 focus:border-black transition-all text-sm font-medium"
                                            placeholder="Candidate ID"
                                        />
                                    </div>

                                    <div className="space-y-1 sm:space-y-1.5">
                                        <label className="text-[9px] sm:text-[10px] font-bold text-black tracking-widest uppercase ml-1">Secret Key</label>
                                        <input
                                            type="password"
                                            required
                                            value={password}
                                    ...
                                    <button type="submit" disabled={isLoading} className="w-full flex justify-center items-center py-3.5 sm:py-4 px-6 rounded-full shadow-2xl text-[9px] sm:text-[10px] font-bold uppercase tracking-[0.2em] text-white bg-black hover:bg-gray-800 transition-all mt-4 sm:mt-6 active:scale-95 disabled:opacity-50">
                        ...
                        <div className="absolute inset-0 backface-hidden bg-white/10 backdrop-blur-2xl p-6 sm:p-10 rounded-[2.5rem] sm:rounded-[3.5rem] shadow-[0_50px_100px_-30px_rgba(0,0,0,0.4)] border border-white/30 flex flex-col"
                            ...
                            <div className="flex-1 flex flex-col mt-6 sm:mt-8">
                                <header className="mb-6 sm:mb-8">
                                    <h2 className="text-2xl sm:text-3xl font-bold text-black tracking-tight mb-2">Security Hub</h2>
                                    <p className="text-gray-600 text-[10px] sm:text-xs font-bold uppercase">Admin Access Terminal</p>
                                </header>

                                <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-5 flex-grow">
                                    <div className="space-y-1 sm:space-y-1.5">
                                        <label className="text-[9px] sm:text-[10px] font-bold text-black tracking-widest uppercase ml-1">Admin Identity</label>
                                        <input
                                            type="text"
                                            required
                                            value={userId}
                                    ...
                                    <div className="space-y-1 sm:space-y-1.5">
                                        <label className="text-[9px] sm:text-[10px] font-bold text-black tracking-widest uppercase ml-1">Access Key</label>
                                        <input
                                            type="password"
                                            required
                                            value={password}
...
const RoleToggle = ({ activeRole, onToggle }: { activeRole: string, onToggle: (role: 'candidate' | 'admin') => void }) => (
    <div className="w-full">
        <label className="text-[9px] sm:text-[10px] font-bold text-black/40 tracking-[0.2em] uppercase mb-3 sm:mb-4 block text-center">
            Identify Profile
        </label>
        <div className="bg-black/30 backdrop-blur-md p-1 rounded-xl sm:rounded-2xl flex items-center border border-white/10 shadow-inner">
            <button
                ...
                className={`flex-1 py-2.5 sm:py-3 px-3 sm:px-4 rounded-lg sm:rounded-xl text-[9px] sm:text-[10px] font-bold uppercase tracking-widest transition-all duration-500 text-white ${
