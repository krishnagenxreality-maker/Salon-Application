             ...
             <div className="max-w-screen-xl mx-auto">
                ...
                <div className="mb-8 sm:mb-12 border-b border-gray-200 dark:border-gray-800 pb-6 sm:pb-8 flex flex-col md:flex-row md:items-end justify-between gap-4">
                    ...
                    <div className="flex flex-wrap items-center gap-2 sm:gap-4">
                        <div className="flex items-center gap-2 px-3 py-1.5 sm:px-4 sm:py-2 bg-gray-100 dark:bg-gray-800 rounded-full text-xs sm:text-sm font-medium text-gray-600 dark:text-gray-400">
                            <CalendarIcon className="w-4 h-4 dark:text-white" />
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 sm:gap-6 mb-12 animate-slide-up">
                    {[
                        ...
                        <div key={idx} className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-6 shadow-sm flex items-center justify-between">
                            ...
                            <div className="h-10 w-10 bg-light-grey dark:bg-gray-700 rounded-full flex items-center justify-center text-black dark:text-white">
                ...
                <div className="animate-slide-up" style={{ animationDelay: '0.2s' }}>
                    <h2 className="text-xl font-bold text-black dark:text-white mb-6">Registered Candidates</h2>
                    
                    {isLoading ? (
                        <div className="text-center py-12 text-gray-500">Loading candidate data...</div>
                    ) : (
                    <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl overflow-hidden shadow-sm">
                        <div className="grid grid-cols-12 bg-gray-50 dark:bg-gray-700/50 p-4 text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider border-b border-gray-200 dark:border-gray-700">
                            ...
                            {users.length === 0 ? (
                                ...
                                users.map((user) => (
                                    <button 
                                        ...
                                        className="w-full grid grid-cols-12 p-4 items-center hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors text-left group"
                                    >
                                        <div className="col-span-3 sm:col-span-2 text-sm font-mono font-medium text-gray-500 dark:text-gray-400">
                                            #{user.id.slice(0,6)}
                                        </div>
                                        <div className="col-span-5 sm:col-span-4">
                                            <div className="flex items-center gap-3">
                                                <div className="h-8 w-8 rounded-full bg-black dark:bg-white text-white dark:text-black flex items-center justify-center text-xs font-bold">
                                                    {(user.name || 'C').charAt(0)}
                                                </div>
                                                <span className="text-sm sm:text-base font-bold text-black dark:text-white">{user.name}</span>
                                            </div>
                                        </div>
                                        <div className="col-span-4 sm:col-span-3 text-sm text-gray-600 dark:text-gray-300 text-right sm:text-left">
                                        ...
                                        <div className="hidden sm:block col-span-1 text-right">
                                            <ChevronRightIcon className="w-5 h-5 text-gray-300 dark:text-gray-600 group-hover:text-black dark:group-hover:text-white transition-colors ml-auto" />
                                        </div>
                                    </button>
                                ))
                            )}
