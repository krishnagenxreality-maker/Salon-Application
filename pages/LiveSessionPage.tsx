...
interface LiveSessionPageProps {
  ...
  onFinishSession: (videoUrl: string) => void;
  onExit: () => void;
  onBack?: () => void;
}
...
const LiveSessionPage: React.FC<LiveSessionPageProps> = ({ serviceName, onStepComplete, onFinishSession, onExit }) => {
  ...
  const handleBegin = async () => {
      try {
        ...
        alert("Camera access is required for live sessions.");
      }
  };
    ...
    <div className="fixed inset-0 w-full h-full flex flex-col animate-fade-in overflow-hidden">
        ...
        <div className="relative z-10 flex-1 flex flex-col lg:flex-row items-center justify-center px-10 sm:px-20 lg:px-32 pt-24 pb-32 overflow-y-auto no-scrollbar gap-10 lg:gap-24">
            ...
            <div className="w-full lg:w-1/2 text-center lg:text-left animate-fade-in-up">
                ...
                <div className="mt-8 flex items-center justify-center lg:justify-start gap-3">
                    ...
                    <span className="text-[10px] font-black text-red-500 uppercase tracking-widest">Recording Session...</span>
                </div>
            </div>
            ...
            <div className="w-full lg:w-1/2 flex justify-center lg:justify-end animate-fade-in-up" style={{ animationDelay: '0.2s' }}>
                <div className="relative w-full aspect-[3/4] max-w-[280px] sm:max-w-[320px] lg:max-w-[360px] bg-black rounded-[2.5rem] overflow-hidden border-2 border-white/10 shadow-[0_40px_100px_rgba(0,0,0,0.8)]">
                    ...
                    <div className="absolute top-6 right-6 px-3 py-1 bg-red-600 text-white text-[8px] font-black uppercase rounded-full">LIVE</div>
                </div>
            </div>
        </div>
        ...
        <footer className="fixed bottom-0 left-0 right-0 h-24 sm:h-28 flex items-center justify-between px-10 sm:px-20 lg:px-32 z-50">
            <div className="flex items-center gap-4 sm:gap-6 h-full">
                <button onClick={onExit} className="text-[9px] sm:text-[10px] font-black text-red-600 hover:text-red-500 uppercase tracking-[0.4em] transition-all whitespace-nowrap drop-shadow-lg">EXIT</button>
                <div className="h-5 w-[1px] bg-white/10" />
                {sessionStartTime && <LiveTimer startTime={sessionStartTime} />}
            </div>
            
            <div className="flex items-center gap-3 sm:gap-4">
                <button onClick={handlePrev} disabled={currentStep === 0} className="h-10 w-10 sm:h-12 sm:w-12 flex items-center justify-center rounded-full border border-white/5 text-white disabled:opacity-0 bg-white/5 backdrop-blur-md hover:bg-white/10 transition-all active:scale-90"><ChevronLeftIcon className="w-4 h-4" /></button>
                <button onClick={handleNext} className="h-11 sm:h-12 px-8 sm:px-10 bg-white text-black rounded-full flex items-center gap-3 hover:scale-105 active:scale-95 transition-all shadow-xl">
                  <span className="text-[8px] sm:text-[9px] font-black uppercase tracking-[0.2em] whitespace-nowrap">{currentStep === totalSteps - 1 ? 'Finish' : 'Next Step'}</span>
